...
// Load environment variables cleanly in server environments
dotenv.config();

const app = express();
const PORT = 3000;

// CORS & Host Header Handling (allows requests from dev, shared preview, and production domains)
...
// Server-side Gemini Client (process.env.GEMINI_API_KEY accessed exclusively on server)
...
// Supported model priority list per official @google/genai guidelines
const GEMINI_MODELS = [
  "gemini-3.8-flash",
...
async function callGemini(options: CallGeminiOptions): Promise<string> {
  ...
  if (!client) {
    const err: any = new Error("GEMINI_API_KEY is not configured in the server environment");
  ...
  for (const modelName of GEMINI_MODELS) {
    try {
      ...
      lastError = err;
      const status = err?.status || err?.statusCode || (err?.message?.includes('429') ? 429 : err?.message?.includes('403') ? 403 : null);
      if (status === 429) {
        ...
        // Bad or unauthorized API key - stop attempting further models
        throw lastError;
      }
    }
  }
...
app.get(["/api/health", "/api/ai/health", "/api/ai-health"], (req, res) => {
  const hasKey = !!process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== 'dummy-key';
  res.json({
    status: "ok",
    aiConfigured: hasKey,
    mode: hasKey ? "live_gemini" : "offline_cfo_ledger",
...
app.post("/api/ai/generate", async (req, res) => {
  try {
    ...
    const status = err?.status === 429 ? 429 : err?.status === 403 ? 403 : 500;
    res.status(status).json({
      error: err?.message || "AI generation failed",
      code: err?.code || (status === 429 ? "RATE_LIMITED" : status === 403 ? "UNAUTHORIZED" : "AI_ERROR"),
      text: "AI service currently unavailable. Please verify API configuration or try again shortly."
    });
  }
});

app.post(["/api/ai/financial-plan", "/api/ai-financial-plan"], async (req, res) => {
  try {
...
app.post(["/api/ai/categorize", "/api/ai-categorize"], async (req, res) => {
  try {
...
app.post(["/api/ai/receipt-scan", "/api/ai-receipt-scan"], async (req, res) => {
  try {
...
`;
    ...
    const rawText = await callGemini({
      prompt: "Extract merchant name, total amount, date, items, and category from this receipt image as JSON.",
      inlineData: {
        mimeType,
        data: base64Data
      },
...
app.post(["/api/ai/emergency-warning", "/api/ai-emergency-warning"], async (req, res) => {
  try {
...
Provide 1-2 punchy sentences of Taglish CFO advice on whether this is an emergency, and how to rapidly replenish it.`;
      ...
      // Graceful degradation when offline or rate-limited
    }
...
app.post("/api/calendar/batch-delete", async (req, res) => {
  try {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return res.status(401).json({ error: "Missing or invalid authorization token" });
    ...
    // Also sweep for any orphaned GoraGo events on primary calendar
    try {
      ...
      for (const item of items) {
        if (item.id && !eventIds.includes(item.id)) {
          try {
            await calendar.events.delete({
              calendarId: "primary",
              eventId: item.id,
    ...
    console.error("Failed to batch delete Google Calendar events:", error);
    res.status(500).json({ error: error?.message || "Failed to batch delete events" });
  }
});
