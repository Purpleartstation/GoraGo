...
import { useSafeCollectionData, saveBill } from '../db';
import type { Account, Bill, RecurringRule } from '../db';
...
export default function BillSheet({ isOpen, onClose }: BillSheetProps) {
  ...
  const isValid = () => {
    if (!name.trim() || !amount || !selectedAccountId) return false;
  ...
  const handleSave = async () => {
    ...
    if (dueType === 'monthly') {
      ...
      await saveBill(newBill, recurringRule);
    } else {
