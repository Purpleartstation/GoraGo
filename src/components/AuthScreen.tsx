...
import { ensureUserProfile, updateUserProfile, autoSyncGoogleCalendarReminders } from '../db';
import { setCalendarToken } from '../utils/googleCalendar';
...
export default function AuthScreen({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // 1. Extract the user's displayName and email directly from Google Auth credentials
      const userEmail = user.email || '';
      const googleDisplayName = user.displayName || userEmail.split('@')[0] || 'Google User';

      // 2. Set email as account email in users/{uid} and default fullName to Google account name if not already set
      ...
      try {
        ...
        if (accessToken) {
          ...
          // Automated calendar sync for bills, debts, and goals
          try {
            await autoSyncGoogleCalendarReminders(userProfile?.householdId || `h_${user.uid}`, accessToken);
          } catch (calErr) {
            console.warn('Auto calendar sync notice on Google sign-in:', calErr);
          }
        }
      } catch (e) {
      ...
      // Preserve customized name if set, otherwise default to Google name
      const effectiveName = (userProfile?.name && userProfile.name.trim() !== '') ? userProfile.name : googleDisplayName;
      setActiveUser({
        id: user.uid,
        name: effectiveName,
        email: userEmail,
      ...
      if (userProfile?.hasPin) {
        await saveLocalSecurityProfile({
          ...
          hasPin: true,
          isGoogleBound: true,
          linkedGoogleEmail: userEmail,
          householdId: userProfile.householdId || `h_${user.uid}`,
        });
  ...
  const handleExistingLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActiveUser({
        id: u.uid,
        name: userProfile?.name || u.displayName || u.email?.split('@')[0] || 'User',
        email: u.email || '',
        isGoogle: false,
      });
      ...
      if (userProfile?.hasPin) {
        await saveLocalSecurityProfile({
          ...
          hasPin: true,
          isGoogleBound: false,
          householdId: userProfile.householdId || `h_${u.uid}`,
        });
        setSessionUnlocked(true);
        if (onComplete) {
          onComplete();
  ...
  const handleFinalizePin = useCallback(async (finalPin: string) => {
    ...
    try {
      ...
      const currentHh = useAppStore.getState().currentHouseholdId || (activeUser?.id ? `h_${activeUser.id}` : 'h_sample');
