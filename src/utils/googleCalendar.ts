...
const STORAGE_CALENDAR_TOKEN_KEY = 'gorago_google_calendar_token';
let cachedAccessToken: string | null = null;
let isSigningIn = false;

export const setCalendarToken = (token: string | null): void => {
  cachedAccessToken = token;
  if (typeof window !== 'undefined') {
    if (token) {
      localStorage.setItem(STORAGE_CALENDAR_TOKEN_KEY, token);
    } else {
      localStorage.removeItem(STORAGE_CALENDAR_TOKEN_KEY);
    }
  }
};
...
export const getCalendarToken = (): string | null => {
  ...
  if (typeof window !== 'undefined') {
    const saved = localStorage.getItem(STORAGE_CALENDAR_TOKEN_KEY);
    if (saved) {
...
/**
 * Batch deletes all calendar events/reminders associated with the household bills and debts
 */
export const cleanupHouseholdCalendarEvents = async (eventIds: string[], customToken?: string): Promise<{ success: boolean; deletedCount: number }> => {
  ...
  try {
    const res = await fetch('/api/calendar/batch-delete', {
      method: 'POST',
      headers: {
    ...
    console.error('Failed to cleanup household calendar events:', err);
    return { success: false, deletedCount: 0 };
  }
};
